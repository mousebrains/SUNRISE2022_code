...
[~,idx_temp] = sort(CTD_combo.dn);

duplicate_idx = find(diff(CTD_combo.dn(idx_temp))==0);

idx_temp(duplicate_idx) = [];

ctd_names = {'C','SP','SA','T','theta','sigma','DO2R','DO2A'};

for k = 1:length(ctd_names)
    CTD_combo.(ctd_names{k}) = CTD_combo.(ctd_names{k})(:,idx_temp);
end

CTD_combo.dn = CTD_combo.dn(idx_temp);
CTD_combo.lat = CTD_combo.lat(idx_temp);
CTD_combo.lon = CTD_combo.lon(idx_temp);
CTD_combo.dist_ctd = CTD_combo.dist_ctd(idx_temp);
CTD_combo.data_num = CTD_combo.data_num(idx_temp);
CTD_combo.profile = CTD_combo.profile(idx_temp);
