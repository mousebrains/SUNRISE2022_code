...
if exist('ctd_combo',"var")
    ...
    [~,idx] = sort(vmp_combo.dn);
    
    %% theta
    temp = [vmp_combo.theta];
    hydro_combo.theta = temp(:,idx);
    ...
    %%  Turbidity
    temp = [vmp_combo.Turbi];
    hydro_combo.Turbi = temp(:,idx);
    
    %%  O2A
    temp = [nan(size(vmp_combo.theta))];
    hydro_combo.DO2A = temp(:,idx);
